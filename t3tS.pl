...
% no required boxes t3test(a2, 0, [(1, 0)], 500).
...
t3testone(ID, N, Boxes, Obstacles, StepsLimit) :-
	...
	format("Simulated position / box here / carries box / program data ~t~72| ....action chosen~n"),
...
t3testonline(N, Boxes, Obstacles, CPos, CarriesBox, ProgramData, StepsLeft, H) :-
	...
	CPos = (X, Y), format("[~w~3|,~w~6|]~7|[~w]~15|[~w]~24|[~w]~t~72| ....", [X, Y, HasBox, CarriesBox, ProgramData]),
%	format('Getting action to perform... '), !,
	(	Obstacles=[_|_], !, t3availables(CPos, Obstacles, Availables),
			t3testperform(ProgramData, HasBox, Availables, Action, ProgramDataUpdated)
		;
		t3testperform(ProgramData, HasBox, [], Action, ProgramDataUpdated), Availables = [north, south, east, west]
	),
